...
var flb_settings = $.extend({
        ...
        next_image_text: 'Nästa &rsaquo;&rsaquo;',
    
        back_image_text: '&lsaquo;&lsaquo; Föregående',
    
        next_and_back_image_text_style: 'font-weight: bold; color: #000; padding:5px;',
    
        image_number_page: 'Bild',
...
flb_loading=setTimeout(function(){
    flb_load_animation()},3000)                                                                                                                     }$(flb_clickclass).click(function(e){e.preventDefault();

flb_image_name=$(this).attr('href');flb_class=$(this).attr('class');

flb_textcontent=$('.'+flb_class+' span').html();flb_lightbox_data=flb_class.split('-');flb_total_images=flb_classamount[flb_lightbox_data[0]];

$("#flb-lightbox-text").html('');if(flb_textcontent!=null&&flb_textcontent!=''&&flb_settings.lightbox_text_status==1){
    if(flb_settings.lightbox_navigation_status==1&&flb_total_images>1)
    {
        $("#flb-lightbox-text").append('<br /><br />')
    }
    $("#flb-lightbox-text").append(flb_textcontent)}$("#flb-lightbox").fadeIn(flb_settings.lightbox_flip_speed/2);

flb_load_animation();

flb_in_img=document.createElement('img');

$("<img />").attr('src',flb_image_name).load(function(){flb_in_img.onload=function(){$("#flb-load").html('');clearTimeout(flb_loading);
...
$("#flb-lightbox-content").hide();setTimeout(function(){
    $(window).resize();
    ...
    flb_distance_from_top=($(window).height()-flb_distance_from_top)/2-(($('#flb-lightbox-text').height()/2)+20)+flb_placement;
        ...
        $("#flb-lightbox-content").prepend('<img id="flb-close" src="'+flb_blank_image+'" width="25px" height="25px" alt="Close" style=" background:#000;margin-top: '+flb_distance_from_top+'px; margin-left: '+flb_total_lightbox_width+'px;'+flb_box_sizing+'"/><br />');
    ...
    setTimeout(function(){
        ...
        if($("#flb-lightbox-text").is(':visible')){
            if(navigator.userAgent.match(/iPad/i)||flb_iphone_check()){
                ...
                $("#flb-lightbox-text").slideToggle()}}},flb_settings.lightbox_flip_speed/2)
    });
    ...
    if(flb_settings.lightbox_navigation_status==1&&flb_total_images>1){
            if(flb_lightbox_data[1]<flb_total_images){
                ...
                if($("#flb-lightbox-text").is(':visible')){
                    if(navigator.userAgent.match(/iPad/i)||flb_iphone_check()){
                        $("#flb-lightbox-text").fadeOut()}else{$("#flb-lightbox-text").slideToggle()
                        }
                }
            e.preventDefault();
            setTimeout(function(){
                $('.'+flb_lightbox_data[0]+'-'+(Math.floor(flb_lightbox_data[1])+1)).trigger('click')},flb_settings.lightbox_flip_speed/2);
                $("#flb-close").attr('src',flb_blank_image);$("#flb-lightbox-image").attr('src',flb_image_name).animate({
                    width:flb_image_innit_width,opacity:0
                },{
                duration:flb_settings.lightbox_flip_speed/2
                })
                ...
                $("#flb-back-pic").click(function(e){
                    ...
                    if($("#flb-lightbox-text").is(':visible')){
                        if(navigator.userAgent.match(/iPad/i)||flb_iphone_check()){
                            ...
                            $("#flb-lightbox-text").slideToggle()}}e.preventDefault();setTimeout(function(){
                                $('.'+flb_lightbox_data[0]+'-'+      (Math.floor(flb_lightbox_data[1])-1)).trigger('click')},flb_settings.lightbox_flip_speed/2);
                    $("#flb-close").attr('src',flb_blank_image);
                    $("#flb-lightbox-image").attr('src',flb_image_name).animate({width:flb_image_innit_width,opacity:0},{
                        duration:flb_settings.lightbox_flip_speed/2
                    })
                })
            }
...
flb_in_img.src=flb_image_name}).error(function(){
    ...
    $(window).resize(function(){flb_i=1;
    
    if($(window).height()<(flb_image_height+$('#flb-lightbox-text').height()+flb_h)+(flb_settings.lightbox_border_width*2)){
        flb_new_height=$(window).height()-(flb_settings.lightbox_border_width*2)-$('#flb-lightbox-text').height()-flb_h;flb_new_width=flb_image_width/flb_image_height*flb_new_height;flb_i=0
    }
